...
use crate::dialect::{
    is_delimited_identifier_start, is_identifier_part, is_identifier_start,
...
impl<'a> Tokenizer<'a> {
    ...
    ///
    /// // Parsing the query
    /// let tokens = Tokenizer::new(&query).tokenize().unwrap();
    ///
    ...
    fn next_token(&self, chars: &mut State) -> Result<Option<Token>, TokenizerError> {
        match chars.peek() {
            Some(&ch) => match ch {
                ...
                '\"' if !is_delimited_identifier_start(ch) && !is_identifier_start(ch) => {
                    let s = self.tokenize_quoted_string(chars, '"')?;
                ...
                quote_start
                    if is_delimited_identifier_start(ch)
                ...
                '0'..='9' | '.' => {
                    ...
                    if exponent_part.is_empty() {
                        let word = peeking_take_while(chars, is_identifier_part);
                ...
                '%' => {
                    ...
                    match chars.peek() {
                        ...
                        Some(sch) if is_identifier_start('%') => {
                    ...
                    match chars.peek() {
                        ...
                        Some('@') => {
                            ...
                            match chars.peek() {
                                ...
                                Some(tch) if is_identifier_start('@') => {
                ...
                ch if is_identifier_start(ch) => self.tokenize_identifier_or_keyword([ch], chars),
